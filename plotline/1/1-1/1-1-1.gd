...
func play() -> void:
	...
	await PlotlineManager.chat("雪影", "……星芦？", ["雪影", "薇芯"])
	await PlotlineManager.chat("薇芯", "（白眼）不许倒过来念。", ["雪影", "薇芯"])
	await PlotlineManager.chat("薇芯", "这可是异世界，有各种魔物和丧尸，这几天可吓死我了。", ["雪影", "薇芯"])
	await PlotlineManager.chat("雪影", "（环顾四周）这房车你带过来的？", ["雪影", "薇芯"])
	await PlotlineManager.chat("薇芯", "没有，路边捡的。上任主人似乎是魔素紊乱死这里面了，费了我好大功夫才清理干净呢。", ["雪影", "薇芯"])
	await PlotlineManager.chat("雪影", "（后背一凉）魔素是啥？", ["雪影", "薇芯"])
	await PlotlineManager.chat("薇芯", "魔力？反正是类似的东西。", ["雪影", "薇芯"])
	await PlotlineManager.chat("薇芯", "我接手了原主人的手稿和资料，这不是一般的房车，咱老家的技术原理在这用不了。", ["雪影", "薇芯"])
	await PlotlineManager.chat("薇芯", "这车的动力就跟这个世界的魔素相关，但手稿上的某些文字我还看不懂。", ["雪影", "薇芯"])
	await PlotlineManager.chat("薇芯", "我需要一点富含魔素的材料来研究原理，我想魔物身上肯定会有，就交给你了雪影！", ["雪影", "薇芯"])
	await PlotlineManager.chat("雪影", "我…我吗？", ["雪影", "薇芯"])
	await PlotlineManager.chat("薇芯", "又寸。房车里有武器来着，还有一些剩下的食物，你降临的地方附近应该有史莱姆。", ["雪影", "薇芯"])
